...
export const TeacherUserPendingEnrollmentList = memo(
  forwardRef<any, Props>(function (
    ...
    const currentTeacher = useMemo(() => {
      ...
      return pendingTeachers[currentIndex];
    }, [pendingTeachers, currentIndex]);

    const [date, email, phoneNumber, fullName] = useMemo(
      () => [
        dayjs(currentTeacher?.enrollment?.createdAt).format('DD-MM-YYYY'),
        currentTeacher?.email,
        currentTeacher ? formatPhoneNumber(currentTeacher?.phoneNumber) : '',
        currentTeacher
          ? generateFullName(
    ...
    const handleNext = useCallback(() => {
      setCurrentIndex((prev) =>
        prev >= pendingTeachers.length - 1 ? prev : prev + 1,
      );
    }, [pendingTeachers]);

    const handleTeacherDetails = useCallback(
      () => onTeacherDetails && onTeacherDetails(currentTeacher),
      [currentTeacher, onTeacherDetails],
    );
        ...
        {currentTeacher && (
          <BaseSurface
            className={cx(
              ...
              onTeacherDetails && 'cursor-pointer hover:!border-primary',
              loading &&
                'pointer-events-none w-full !items-center !justify-center',
            )}
            rounded='sm'
            onClick={handleTeacherDetails}
            role={onTeacherDetails ? 'button' : 'article'}
          >
            {loading ? (
              <BaseSpinner size='sm' />
            ) : (
